                ...
                <div className="fields">
                  <div className="bookMetaFields full">
                    <label className="countField">
                      ...
                      <span className={`countValue ${form.total_count ? `hasCount digits${Math.min(3, String(form.total_count).length)}` : "isEmpty"}`}><input type="text" inputMode="numeric" placeholder="비어 있음" value={form.total_count || ""} onChange={(e) => field("total_count", Number(e.target.value.replace(/\D/g, "")))} /><span className="countUnit"><span aria-hidden="true">{form.count_unit || "권"}</span><select aria-label="수량 단위" value={form.count_unit || "권"} onChange={(e) => changeCountUnit(e.target.value as "권" | "화")}><option>권</option><option>화</option></select></span></span>
                    </label>
