...
export default function BookExcerpts({ bookId, title }: { bookId: string; title: string }) {
  ...
  return <section className={`bookExcerpts imageExportExclude${!loading && !items.length && !error && !progress ? ' isEmpty' : ''}`} aria-label="발췌 이미지">
    <header><h3>EXCERPTS <span>{items.length || ''}</span></h3><label className="excerptAdd"><ImagePlus size={15} />{progress || '이미지 추가'}<input type="file" accept="image/*" multiple disabled={loading || !!progress} onChange={event => { void add(event.target.files); event.target.value = ''; }} /></label></header>
