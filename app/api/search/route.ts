...
async function kakao(q:string):Promise<SearchBook[]> {
  try {
    ...
    return (Array.isArray(items)?items:[]).map((item:Record<string,unknown>)=>{const id=String(item.id||item.series_id||'');const thumb=String(item.thumbnail||item.image||'');return {title:String(item.title||''),author:String(item.authors||item.author||item.author_name||''),cover:thumb.startsWith('http')?thumb:thumb?`https://dn-img-page.kakao.com/download/resource?kid=${encodeURIComponent(thumb)}`:'',url:id?`https://page.kakao.com/content/${id}`:'',totalCount:positiveCount(item.on_sale_count,item.total_count,item.slide_count,item.book_count),category:category(`${item.sub_category||''} ${item.category||''}`),platform:'카카오페이지'};});
  } catch { return []; }
}
...
export async function GET(req:NextRequest) {
  ...
  const normalize=(text:string)=>text.toLowerCase().replace(/\[[^\]]*\]|\([^)]*\)|단행본|개정판|완전판|외전|특별/g,'').replace(/[^0-9a-z가-힣]/g,'');
